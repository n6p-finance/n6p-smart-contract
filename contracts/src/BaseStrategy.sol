...
interface VaultAPI is IERC20 {
     ...
     * Controller. It is critical that this call is handled as intended by the
...
/**
 * This interface is here for the lunar bot to use.
 */
interface StrategyAPI {
    ...
    function lunar() external view returns (address);
 ...
 *  closely with the Controller contract. This contract should be inherited and the
...
abstract contract BaseStrategy {
     ...
     * @param _lunar The adddress of the _lunar. _lunar
    ...
    function _initialize(
        ...
        want.approve(_vault, type(uint256).max); // Give Vault unlimited access (might save gas)
    ...
    /**
     * @notice
     *  Used to change `lunar`.
     *
     *  `lunar` is the only address that may call `tend()` or `harvest()`,
     *  other than `governance()` or `strategist`. However, unlike
     *  `governance()` or `strategist`, `lunar` may *only* call `tend()`
     *  and `harvest()`, and no other authorized functions, following the
     *  principle of least privilege.
     *
     *  This may only be called by governance or the strategist.
     * @param _lunar The new address to assign as `lunar`.
     */
    function setKeeper(address _lunar) external onlyAuthorized {
        require(_lunar != address(0));
        lunar = _lunar;
        emit UpdatedKeeper(_lunar);
    }
    ...
    /**
     * @notice
     *  Used to automatically trigger a harvest by our lunar. Can be
    ...
    /**
     * @notice
     *  Provide a signal to the lunar that `tend()` should be called. The
     *  lunar will provide the estimated gas cost that they would pay to call
     *  `tend()`, and this function should use that estimate to make a
     *  determination if calling it is "worth it" for the lunar. This is not
     *  the only consideration into issuing this trigger, for example if the
     *  position would be negatively affected if `tend()` is not called
     *  shortly, then this can return `true` even if the lunar might be
     ...
     *  time.
     * @param callCostInWei The lunar's estimated gas cost to call `tend()` (in wei).
     ...
     *
     *  This may only be called by governance, the strategist, or the lunar.
     */
    ...
    /**
     * @notice
     *  Provide a signal to the lunar that `harvest()` should be called. The
     *  lunar will provide the estimated gas cost that they would pay to call
     *  `harvest()`, and this function should use that estimate to make a
     *  determination if calling it is "worth it" for the lunar. This is not
     *  the only consideration into issuing this trigger, for example if the
     *  position would be negatively affected if `harvest()` is not called
     *  shortly, then this can return `true` even if the lunar might be "at a
     ...
     * @param callCostInWei The lunar's estimated gas cost to call `harvest()` (in wei).
...
abstract contract BaseStrategyInitializable is BaseStrategy {
    ...
    function initialize(
        ...
        _initialize(_vault, _strategist, _rewards, _lunar);
    }
    ...
    function clone(
        ...
        BaseStrategyInitializable(newStrategy).initialize(_vault, _strategist, _rewards, _lunar);
